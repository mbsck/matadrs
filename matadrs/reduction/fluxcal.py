...
def plot_spec(
    ...
    wl_lim: Tuple[float, float] = (np.nan, np.nan),
    xlog: bool = False,
    ylog: bool = False,
) -> None:
    ...
    if match:
        ...
        plt.title(
            cal_name_db + r", diam = $%.2f \pm %.2f$ mas" % (diam_cal, diam_err_cal)
        )
...
def plot_skycalc_output(
    fpath_in: Path, figpath_out: Path, airmass: float, pwv: float
) -> None:
...
def fluxcal(
    ...
    calc_spectrum_offset: bool = False,
) -> int:
    ...
    if do_airmass_correction:
        ...
        create_skycalc_inputfile(
            fname_in_sci, airmass_sci, pwv_sci, wmin, wmax, wdelta=dlambda
        )
        print("Start SkyCalc (SCI).")
        subprocess.check_call(
            ["skycalc_cli", "-i", fname_in_sci, "-o", fname_skycalc_out_sci]
        )
        ...
        create_skycalc_inputfile(
            fname_in_cal, airmass_cal, pwv_cal, wmin, wmax, wdelta=dlambda
        )
        print("Start SkyCalc (CAL).")
        subprocess.check_call(
            ["skycalc_cli", "-i", fname_in_cal, "-o", fname_skycalc_out_cal]
        )
    ...
    if mode == "flux" or mode == "both":
        ...
        try:
            for j in range(len(inhdul_sci["OI_FLUX"].data["FLUXDATA"])):
                flux_raw_sci = inhdul_sci["OI_FLUX"].data["FLUXDATA"][
                    j
                ]  # *np.exp(airmass_sci)
                flux_raw_cal = inhdul_cal["OI_FLUX"].data["FLUXDATA"][
                    j
                ]  # *np.exp(airmass_cal)
    ...
    if mode == "corrflux" or mode == "both":
        ...
        for j in range(len(inhdul_sci["OI_VIS"].data["VISAMP"])):
            ...
            corrflux_raw_sci = inhdul_sci["OI_VIS"].data["VISAMP"][
                j
            ]  # *np.exp(airmass_sci)
            ...
            corrflux_raw_cal = inhdul_cal["OI_VIS"].data["VISAMP"][
                idx_cal
            ]  # *np.exp(airmass_cal)
...
def update_corrflux_from_vis2(inputfile: Path, outputfile: Path, flux_Jy: int) -> None:
...
def update_vis2_from_corrflux(inputfile: Path, outputfile: Path, total_flux) -> None:
...
def transform_spectrum_to_real_spectral_resolution(
    ...
    figpath_out: Path | str = "spectrum_plots.png",
    ylabel: str = "",
    yscale: str = "none",
) -> np.ndarray:
...
def calc_corr_offset(
    spectrum1: ArrayLike, spectrum2: ArrayLike, shift_max: int
) -> List[float]:
...
def plot_corr_offset(
    rp_list: List[float], x: List[float], figpath_out: Union[str, Path]
) -> None:
