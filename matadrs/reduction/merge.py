...
from ..utils.tools import cprint, flip_phases, split_fits
...
def get_output_file_path(
    fits_file: Path, output_dir: Path, chopped: bool = False
) -> Path:
    ...
    if chopped:
        return (
            output_dir
            / f"{readout.name}_{tpl_start_cal}:{tpl_start_sci}_{detector}_FINAL_TARGET_INT_CHOPPED.fits"
        )
    return (
        output_dir
        / f"{readout.name}_{tpl_start_cal}:{tpl_start_sci}_{detector}_FINAL_TARGET_INT.fits"
    )
...
def get_averaged_files(directories: List[Path], chopped: bool = False):
...
def execute_multiple_merges(
    fluxes: List[Path], visibilities: List[Path], chopped: bool, output_dir: Path
) -> None:
...
def prepare_multiple_merges(directories: Path, output_dir: Path):
    ...
    coherent_dir, incoherent_dir = map(
        lambda x: Path(str(x).replace("averaged", "calib").replace("-AVG", "")),
        directories,
    )
    coherent_unchopped_vis, coherent_chopped_vis = split_fits(coherent_dir, vis_tag)
    coherent_unchopped_flux, coherent_chopped_flux = split_fits(coherent_dir, flux_tag)
    incoherent_unchopped_vis, incoherent_chopped_vis = split_fits(
        incoherent_dir, vis_tag
    )
    incoherent_unchopped_flux, incoherent_chopped_flux = split_fits(
        incoherent_dir, flux_tag
    )

    execute_multiple_merges(
        [coherent_unchopped_flux, incoherent_unchopped_flux],
        [coherent_unchopped_vis, incoherent_unchopped_vis],
        False,
        output_dir,
    )
    execute_multiple_merges(
        [coherent_chopped_flux, incoherent_chopped_flux],
...
def execute_merge(
    ...
    bcd_visibilities: List[Path] | List[str] | None = None,
    bcd_pip_visibilities: List[Path] | List[str] | None = None,
    chopped: bool = False,
    index: int = None,
) -> None:
    ...
    oifits_patchwork(
        list(map(str, files_to_merge)),
        str(out_file),
        oi_types_list=OI_TYPES,
...
def merge_averaged_files(
    coherent_dir: Path, incoherent_dir: Path, output_dir: Path
) -> None:
...
def merge_folders(
    coherent_dirs: List[Path], incoherent_dirs: List[Path], output_dir: Path
) -> None:
    ...
    for coherent_dir, incoherent_dir in zip(coherent_dirs, incoherent_dirs):
        cprint(
            "Merging files of folder" f" {coherent_dir.name.split('/')[~0]}...", "lp"
        )
        cprint(f"{'':-^50}", "lg")
...
def merging_pipeline(averaged_dir: Path) -> None:
    ...
    incoherent_dirs = [
        Path(str(directory).replace("coherent", "incoherent"))
