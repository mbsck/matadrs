...
def avg_oifits(
    ...
    outfile_path: Path,
    headerval: List[str] = [],
    avg_baselines: bool = False,
    avg_groups: List[int] = [6],
    filter_lst_flux: List = [],
    filter_lst_corrflux: List = [],
    scale_flux: bool = False,
    scale_corrflux: bool = False,
    scale_wl_range: List[float] = [8.3, 9.2],
    avg_func: str = "biweightmean",
) -> None:
    """Averages (.fits)-files.
...
def oifits_patchwork(
    infile_list: List[Path],
    outfile_path: Path,
    oi_types_list: List[List[str]] = [["vis2", "visamp", "visphi", "t3", "flux"]],
    headerval: List[str] = [],
) -> None:
    """
...
def oifits_restrict_wavelengths(
    ...
    if do_flag:
        ...
        outhdul["OI_WAVELENGTH"].data["EFF_BAND"] = hdu["OI_WAVELENGTH"].data[
            "EFF_BAND"
        ][idx]
